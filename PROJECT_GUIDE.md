...
PathPlanner 是整个模块化闭环微操作系统中的最上层运动学与轨迹规划节点。
...
这意味着 PathPlanner 的核心是“基于 Simulink / Simscape Multibody 的模型驱动运动学求解”。其余代码主要是在为这条求解链路提供运行时编排、配置管理、通信封装和可视化界面。
...
- 进程内：PathPlanner 内部是由多个职责单一的 MATLAB 类协作完成。
...
这里的设计重点是通过模型仿真把任意闭环机构描述变成可复用求解器”。
...
自动模式是 App 层的一个关键体验特性。其逻辑是把现有的手动按钮流程串起来：
...
PathPlanner 之所以能够支持不同模块化机构，是因为它约定了“模型输入输出接口”。
...
从系统级视角看，PathPlanner 是整个模块化闭环微操作系统中的最上层运动学与轨迹规划节点。它与 Controller Panel 共同构成分布式控制架构中的两个关键软件端点：
...
这意味着 PathPlanner 的本质是一个“基于模型的运动学求解服务 + 交互式前端”。
...
在 MATLAB 进程内部，PathPlanner 是由一组面向职责拆分的类协作组成：
...
GUI 中的 Automatic Mode 是把已经存在的三步流程串联起来：
...
`PathPlanner.mlapp` 是用户唯一直接接触的界面，是一个带状态机的事件驱动前端。
...
`src/Callbacks/` 下的文件是 GUI 与求解器之间的适配层。
...
`src/Models/` 中的 `slx` 文件是机构知识的核心承载体。
...
PathPlanner 最重要的扩展任务，通常是接入一种新的模块化机构构型。推荐按下面顺序做：
...
从维护角度，PathPlanner 最核心的是下面三件事的组合：
...
Simulink 依赖 Base Workspace 这一事实决定了 `ConfigManager` 与 `WorkspaceManager` 是核心基础设施。
